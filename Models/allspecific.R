...
#constuct panelPOMP
shared_parameter =numeric(0)
names(shared_parameter) = c()

specific_mat = matrix(data = c(rep(0.875100208, 10), rep( 0.0041770577,10), rep(0.004621913,10),
                               rep(7.991700e-04,10),rep(303.5295,10),rep(0.0007095320,10),rep(8.143122e-06,10)
                               ), nrow = 7, nc = 10, byrow = T, 
                      dim = list(rownames = c("alpha","Beta", "gamma", "Theta","kf","sigB","sigG"),
...
names.of.estimated.shared.params <-  NULL
names.of.estimated.specific.params <- c("alpha","Beta", "gamma", "Theta","kf","sigB","sigG")
dent_rw.sd= 0.00125
parameter_candidates = list(shared_parameter, specific_mat)
names(parameter_candidates)=c("shared", "specific")
U = length(panelfood)

bake(file = "specific.rds",
     seed = 334388458L,
     kind = "L'Ecuyer" ,
     {
       foreach(
