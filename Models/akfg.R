...
#library("Rmpi"); library("doMPI"); cl <- startMPIcluster(); registerDoMPI(cl)
#doParWorkers <- getDoParWorkers()
#cat("R started a cluster of size: getDoParWorkers() =",doParWorkers,"\n")
...
trails = list(trail1,trail2, trail3,trail4,trail5, trail6, trail7, trail8, trail9, trail10)
...
shared_parameter = c(0.0041770577,7.991700e-04,0.0007095320,8.143122e-06)
names(shared_parameter) = c("Beta","Theta","sigB","sigG")

specific_mat = matrix(data = c(rep(0.875100208, 10), rep(303.5295,10),rep(0.004621913,10)), nrow = 3, nc = 10, byrow = T, 
                      dim = list(rownames = c("alpha", "kf","gamma"),
...
names.of.estimated.shared.params <-  c("Beta","Theta","sigB","sigG")
names.of.estimated.specific.params <- c("alpha", "kf","gamma")
dent_rw.sd= 0.00125
parameter_candidates = list(shared_parameter, specific_mat)
names(parameter_candidates)=c("shared", "specific")
U = length(panelfood)

bake(file = "akfg_mif.rds",
     seed = 334388458L,
     kind = "L'Ecuyer" ,
     {
       foreach(
