...
specific_mat = matrix(data = numeric(0), nr = 0, nc = 10, 
                      dim=list(NULL, colnames=c("u1","u2","u3","u4","u5", "u6","u7","u8","u9","u10")))
...
names.of.estimated.shared.params <-  c("alpha","Beta", "Theta", "gamma",  "sigG", "sigB", "kf")
names.of.estimated.specific.params <- NULL
dent_rw.sd= 0.00125
parameter_candidates = list(shared_parameter, specific_mat)
names(parameter_candidates)=c("shared", "specific")
U = length(panelfood)

bake(file = "all_mif.rds",
     seed = 334388458L,
     kind = "L'Ecuyer" ,
     {
       foreach(
...
save(mf,lls,best,file="all_mif.RData")
